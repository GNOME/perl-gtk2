...
reference to an array of them instead.  Anonymous arrays are useful htere, and
an empty anonymous array is a simple way to say 'no flags'.
...
You don't need to specify string lengths, although string length parameters
should still be available for functions dealing with binary strings.

Anything that uses GError in C will croak on failure, setting $@ to the
returned error message.  The ideology here is that GError is to be used for
...
the bindings do automatic memory management.  you never need to use these.
...
Gtk2::Helper has a wrapper for Glib::IOChannel->add_watch which
makes it behave more like gtk_input_add.
...
number of elements in that list, will normally omit the number of elements and
just accept a variable number of arguments that will be converted into the list
and number of elements. In many instances parameters have been reordered so
that this will work. See below for exceptions and specific cases.
...
list could be used.  $list->insert_items($position, $item1, $item2, ...) 
...
to get the model, try $selection->get_tree_view->get_model.
