...
All the scrollbar classes (Gtk2:ScrollBar, Gtk2::HScrollBar,
Gtk2::VScrollBar) are written with a capital C<B> as opposed to a
lowercase C<b> that is used in gtk2+ itself.

=item Gtk2::Gdk::Pixbuf::render_pixmap_and_mask

=item Gtk2::Gdk::Pixbuf::render_pixmap_and_mask_for_colormap

These two methods don't take the pixmap and bitmap as arguments, instead
they return the pixmap in scalar context and both pixmap and bitmap in
list context, i.e.:

  my $pixmap = $pixbuf->render_pixmap_and_mask (0.5);
  my ($pixmap, $bitmap) = $pixbuf->render_pixmap_and_mask (0.5);
...
=item Gtk2::Gdk::PixbufAnimation::get_iter

=item Gtk2::Gdk::PixbufAnimationIter::advance

These functions take a GTimeVal in C; the GTimeVal is like a struct timeval
in the unix standard library.  Perl 5.8.0 includes Time::HiRes, which returns
the seconds and microseconds numbers that would go into a timeval, so
get_iter and advance take those values instead.  If you omit all of them,
Gtk2 will use the current time.

  $animation->get_iter ($seconds, $microseconds);
  $animation->get_iter;
  $animiter->advance ($seconds, $microseconds);
  $animiter->advance;
