...
The intersection parameter is ommited. If called in scalar context the return 
value is TRUE or FALSE based on the return value of the c function. If called 
in array context the same boolean is return and in addition if that boolean is
TRUE a Gtk2::Gdk::Rectangle object coresponding with the intersection paramter 
is returned.
